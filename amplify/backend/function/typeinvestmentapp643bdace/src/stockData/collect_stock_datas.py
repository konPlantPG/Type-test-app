...
API_URL = os.getenv("API_TYPEINVESTMENTAPP_GRAPHQLAPIENDPOINTOUTPUT")
API_KEY = os.getenv("API_TYPEINVESTMENTAPP_GRAPHQLAPIKEYOUTPUT")
...
def collect(start_rate, end_rate):
    ...
    for ticker in tqdm(df_data_j['コード'][start_index:end_index]):
        ...
        try:
            ...
            mutation = {
                'query': '''
                    mutation CreateStock($id: ID!, $code: String!, $name: String!, $price: Float, $dividend: Float, $createdAt: AWSDateTime!) {
                    createStock(input: {id: $id, code: $code, name: $name, price: $price, dividend: $dividend, createdAt: $createdAt}) {
                        id
                        code
                        name
                        price
                ...
                'variables': {
                    'id': str(uuid.uuid1()),
                    'code': str(ticker),  # この行をvariablesオブジェクト内に移動しました
                    'name': stock_info['銘柄名'],
                    'price': stock_info['株価'],
                    'dividend': stock_info['配当'],  # 重複していた行を削除しました
